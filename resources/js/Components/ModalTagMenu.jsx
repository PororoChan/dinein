...
import FormTag from "@/Pages/admin/tagmenu/FormTag";
...
const ModalTagMenu = ({id, link, icon, title, msg, btn, form, show, hide, type, column, tag}) => {
    const [isToast, setIsToast] = useState(false)
    const [toastM, setToast] = useState([])
    
    const dtDelete = (ids) => {
        hide()
        axios.delete('tags/delete' + '/' + ids)
        .then(res => {
                ...
                <div className="modal-dialog" role="document">
                    <div className="modal-content">
                        ...
                        <div className="modal-body">
                            {(type != 'delete' ? <FormTag id={id} click={hide} btn={btn} type={type} column={column} tag={tag}/> : '')}
                            {(msg != '' ? msg : '')}
                        </div>
